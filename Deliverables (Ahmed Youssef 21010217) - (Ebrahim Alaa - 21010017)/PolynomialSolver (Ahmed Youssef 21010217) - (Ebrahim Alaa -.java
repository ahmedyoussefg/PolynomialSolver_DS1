...
public class PolynomialSolver implements IPolynomialSolver {
            ...
            while (read.hasNext()) {
                switch (read.next()) {
                    ...
                    case "add":
                        ...
                        if (just_cleared[key2 - 65] || just_cleared[key - 65]) {
                            System.out.println("Error");
                            return;
                        }
                        int[][] r = pol.add(key, key2);
                        for (int i = 0; i < r.length; i++) {
                            SingleLinkedList.Node r_coff = new SingleLinkedList.Node();
                            r_coff.element = r[i][0];
                            SingleLinkedList.indx = 3;
                            SingleLinkedList.list_R.add(r_coff);
                        }
                        System.out.println(pol.print('R'));
                        pol.clearPolynomial('R');
                        break;
                    case "sub":
                        key = read.next().charAt(0);
                        key2 = read.next().charAt(0);
                        if (key != 'A' && key != 'B' && key != 'C') {
                            System.out.println("Error");
                            return;
                        }
                        ...
                        int[][] r_s = pol.subtract(key, key2);
                        for (int i = 0; i < r_s.length; i++) {
                            SingleLinkedList.Node r_coff = new SingleLinkedList.Node();
                            r_coff.element = r_s[i][0];
                            SingleLinkedList.indx = 3;
                            SingleLinkedList.list_R.add(r_coff);
                        }
                        System.out.println(pol.print('R'));
                    ...
                    case "mult":
                        ...
                        int[][] r_m = pol.multiply(key, key2);
                        for (int i = 0; i < r_m.length; i++) {
                            SingleLinkedList.Node r_coff = new SingleLinkedList.Node();
                            r_coff.element = r_m[i][0];
                            SingleLinkedList.indx = 3;
                            SingleLinkedList.list_R.add(r_coff);
                        }
                        System.out.println(pol.print('R'));
                    ...
                    case "eval":
                        ...
                        float result = pol.evaluatePolynomial(key, a);
                        if (result - Math.floor(result) == 0 ) // whole number
                        {
    ...
    public String print(char poly) {
        ...
        switch (poly) {
            ...
            case 'R':
                SingleLinkedList.indx = 3;
                int len4 = SingleLinkedList.list_R.size();
                SingleLinkedList.Node coffs4 = new SingleLinkedList.Node();
                for (int i = 0; i < len4; i++) {
                    SingleLinkedList.indx = 3;
                    coffs4 = (SingleLinkedList.Node) SingleLinkedList.list_R.get(i);
                    int digit = coffs4.element;
                    if (i != 0 && digit > 0 && !printed.equals("")) {
                        printed += "+";
                    }
                    if (digit == 0) {
    ...
    public float evaluatePolynomial(char poly, float value) {
        ...
        switch (poly)
        {
            case 'A':
                SingleLinkedList.indx = 0;
                int size1 = SingleLinkedList.list_A.size();
                for (int i = 0; i < size1; i++) {
                    SingleLinkedList.Node elem = (SingleLinkedList.Node) new SingleLinkedList().get(i);
                    digit = elem.element;
                    power = size1 - i - 1;
                    result += digit * Math.pow(value, power);
                }
                return result;
            case 'B':
                SingleLinkedList.indx = 1;
                int size2 = SingleLinkedList.list_B.size();
            ...
            case 'C':
                SingleLinkedList.indx = 2;
                int size3 = SingleLinkedList.list_C.size();
    ...
    public int[][] add(char poly1, char poly2) {
        ...
        if (size1 >= size2) {
            ...
            for (int i = 0, j = 0; i < size1; i++) {
                ...
                SingleLinkedList.indx = poly2 - 65;
                if (letter_2 == 0)
                    Temp2 = (SingleLinkedList.Node) SingleLinkedList.list_A.get(j);
                else if (letter_2 == 1)
                    Temp2 = (SingleLinkedList.Node) SingleLinkedList.list_B.get(j);
                else if (letter_2 == 2)
                    Temp2 = (SingleLinkedList.Node) SingleLinkedList.list_C.get(j);
                j++;
        ...
        } else // if size2 > size1
        {
            ...
            for (int i = 0, j = 0; i < size2; i++) {
                ...
                SingleLinkedList.indx = poly1 - 65;
                if (letter_1 == 0)
                    Temp1 = (SingleLinkedList.Node) SingleLinkedList.list_A.get(j);
                else if (letter_1 == 1)
                    Temp1 = (SingleLinkedList.Node) SingleLinkedList.list_B.get(j);
                else if (letter_1 == 2)
                    Temp1 = (SingleLinkedList.Node) SingleLinkedList.list_C.get(j);
                SingleLinkedList.indx = poly2 - 65;
                j++;
                if (letter_2 == 0)
    ...
    public int[][] multiply(char poly1, char poly2) {
        ...
        int[][] result = new int[size1 + size2 - 1][2];

        for (int i = 0, counter = 0; i < size1 && counter < size1*size2; i++) {
            for (int j = 0; j < size2; j++) {
                ...
                if (letter_2 == 0)
                    ...
                    Temp2 = (SingleLinkedList.Node) SingleLinkedList.list_C.get(j);
                power2 = size2 - j - 1;
                int number = (size1 + size2 - 2) - (power1 + power2);
                result[number][0] += Temp1.element * Temp2.element;
                result[number][1] = power1 + power2;
                counter++;
            }
        }
        return result;
